...
diff -q base base_original | grep -oP '(?<=Only in base_original: ).*' | xargs -t -I '{}' cp base_original/'{}' base/'{}';


#/ja/assets/ should contain the assets*.pk3 files (and any mod files you've included) from the bind mount
#copy them to the new /ja/base/ directory if they aren't already there
diff -q assets base | grep -oP '(?<=Only in assets: ).*' | xargs -t -I '{}' cp assets/'{}' base/'{}';
...
diff -q base assets | grep -oP '(?<=Only in base: ).*' | grep -vP 'cgamei386.so|jampgamei386.so|uii386.so' | xargs -t -I '{}' rm base/'{}';

#check that the 4 assets{0-3}.pk3 files are present in /ja/base/ directory
if test -f "base/assets0.pk3"; then
    if test -f "base/assets1.pk3"; then
        if test -f "base/assets2.pk3"; then
            if test -f "base/assets3.pk3"; then
